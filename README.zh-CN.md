...
管理 A/B 快照的持久化存储。同步 Worker 调用 `save()` 写入，请求路径客户端调用 `load()` / `metadata()` 读取。
...
interface ABSpecStoreInterface
{
    ...
    /**
     * 返回快照元数据。updatedAtMs 是最后一次保存的毫秒时间戳；
     * SDK 用它来判断数据新鲜度。无数据时传 null。
     */
...
- `metadata()` 中的 `updatedAtMs` 应为 `save()` 时捕获的毫秒时间戳 — 例如 `(int)(microtime(true) * 1000)`
